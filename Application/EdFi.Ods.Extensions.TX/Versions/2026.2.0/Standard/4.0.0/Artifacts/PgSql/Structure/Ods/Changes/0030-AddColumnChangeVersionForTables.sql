...
IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='tx' AND table_name='coursetranscriptext' AND column_name='changeversion') THEN
ALTER TABLE tx.CourseTranscriptExt ADD ChangeVersion BIGINT DEFAULT (0) NOT NULL;
ALTER TABLE tx.CourseTranscriptExt ALTER ChangeVersion SET DEFAULT nextval('changes.ChangeVersionSequence');
END IF;
