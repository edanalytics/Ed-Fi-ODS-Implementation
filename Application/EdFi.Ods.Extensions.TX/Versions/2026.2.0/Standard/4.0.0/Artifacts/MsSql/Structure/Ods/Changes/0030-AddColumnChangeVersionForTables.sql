...
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[tx].[CourseTranscriptExt]') AND name = 'ChangeVersion')
BEGIN
ALTER TABLE [tx].[CourseTranscriptExt] ADD [ChangeVersion] [BIGINT] CONSTRAINT CourseTranscriptExt_DF_ChangeVersion DEFAULT (0) NOT NULL;
ALTER TABLE [tx].[CourseTranscriptExt] DROP CONSTRAINT CourseTranscriptExt_DF_ChangeVersion;
ALTER TABLE [tx].[CourseTranscriptExt] ADD CONSTRAINT CourseTranscriptExt_DF_ChangeVersion DEFAULT (NEXT VALUE FOR [changes].[ChangeVersionSequence]) For [ChangeVersion];
END
...
ALTER TABLE [tx].[PriorYearSSAOrgAssociationExt] DROP CONSTRAINT PriorYearSSAOrgAssociationExt_DF_ChangeVersion;
ALTER TABLE [tx].[PriorYearSSAOrgAssociationExt] ADD CONSTRAINT PriorYearSSAOrgAssociationExt_DF_ChangeVersion DEFAULT (NEXT VALUE FOR [changes].[ChangeVersionSequence]) For [ChangeVersion];
END
