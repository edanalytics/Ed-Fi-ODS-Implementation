...
DO $$
BEGIN
IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='tx' AND table_name='actualext' AND column_name='changeversion') THEN
ALTER TABLE tx.ActualExt ADD ChangeVersion BIGINT DEFAULT (0) NOT NULL;
ALTER TABLE tx.ActualExt ALTER ChangeVersion SET DEFAULT nextval('changes.ChangeVersionSequence');
END IF;
