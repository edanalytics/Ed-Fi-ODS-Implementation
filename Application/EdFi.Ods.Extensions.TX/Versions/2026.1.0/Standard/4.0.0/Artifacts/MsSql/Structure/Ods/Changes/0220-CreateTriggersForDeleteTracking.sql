...
DROP TRIGGER IF EXISTS [tx].[tx_PreschoolOutcomesReportingDescriptor_TR_DeleteTracking]
GO

CREATE TRIGGER [tx].[tx_PreschoolOutcomesReportingDescriptor_TR_DeleteTracking] ON [tx].[PreschoolOutcomesReportingDescriptor] AFTER DELETE AS
BEGIN
    IF @@rowcount = 0 
        RETURN
    ...
    INSERT INTO [tracked_changes_edfi].[Descriptor](OldDescriptorId, OldCodeValue, OldNamespace, Id, Discriminator, ChangeVersion)
    SELECT  d.PreschoolOutcomesReportingDescriptorId, b.CodeValue, b.Namespace, b.Id, 'tx.PreschoolOutcomesReportingDescriptor', (NEXT VALUE FOR [changes].[ChangeVersionSequence])
    FROM    deleted d
            INNER JOIN edfi.Descriptor b ON d.PreschoolOutcomesReportingDescriptorId = b.DescriptorId
END
GO

ALTER TABLE [tx].[PreschoolOutcomesReportingDescriptor] ENABLE TRIGGER [tx_PreschoolOutcomesReportingDescriptor_TR_DeleteTracking]
GO
