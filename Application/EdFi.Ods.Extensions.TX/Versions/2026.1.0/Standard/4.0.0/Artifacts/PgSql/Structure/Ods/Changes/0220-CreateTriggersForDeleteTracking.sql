...
CREATE OR REPLACE FUNCTION tracked_changes_tx.preschooloutcomesreportingdescriptor_deleted()
    RETURNS trigger AS
$BODY$
BEGIN
    INSERT INTO tracked_changes_edfi.descriptor(olddescriptorid, oldcodevalue, oldnamespace, id, discriminator, changeversion)
    SELECT OLD.PreschoolOutcomesReportingDescriptorId, b.codevalue, b.namespace, b.id, 'tx.PreschoolOutcomesReportingDescriptor', nextval('changes.ChangeVersionSequence')
    FROM edfi.descriptor b WHERE old.PreschoolOutcomesReportingDescriptorId = b.descriptorid ;

    RETURN NULL;
END;
$BODY$ LANGUAGE plpgsql;

IF NOT EXISTS(SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trackdeletes' AND event_object_schema = 'tx' AND event_object_table = 'preschooloutcomesreportingdescriptor') THEN
CREATE TRIGGER TrackDeletes AFTER DELETE ON tx.preschooloutcomesreportingdescriptor 
    FOR EACH ROW EXECUTE PROCEDURE tracked_changes_tx.preschooloutcomesreportingdescriptor_deleted();
END IF;
